var n=document.querySelectorAll(".drum").length;
for(var i=0;i<n;i++)
{
  document.querySelectorAll("button")[i].addEventListener("click",function(){var xt=this.innerHTML;makesound(xt);animation(xt);});
}
document.addEventListener("keypress",function(event){var tst=event.key;makesound(tst);animation(tst);})
function makesound(wer){
switch(wer)
{
  case "w":
    var audio=new Audio("sounds/tom-1.mp3");
    audio.play();
    document.querySelector("h1").style.color="red";
   break;
  case "a":
      var x=new Audio("sounds/tom-2.mp3");
      x.play();
      document.querySelector("h1").style.color="black";
   break;
    case "s":
       var x=new Audio("sounds/tom-3.mp3");
       x.play();
       document.querySelector("h1").style.color="blue";
     break;
     case "d":
        var x=new Audio("sounds/tom-4.mp3");
        x.play();
        document.querySelector("h1").style.color="green";
      break;
      case "j":
         var x=new Audio("sounds/snare.mp3");
         x.play();
         document.querySelector("h1").style.color="yellow";
       break;
       case "k":
          var x=new Audio("sounds/crash.mp3");
          x.play();
          document.querySelector("h1").style.color="orange";
        break;
        case "l":
           var x=new Audio("sounds/kick-bass.mp3");
           x.play();
           document.querySelector("h1").style.color="purple";
         break;
    default:
    console.log(tx);
}
}
function animation(key)
{
  var activebutton = document.querySelector("."+key);
  activebutton.classList.add("pressed");
  setTimeout(function(){activebutton.classList.remove("pressed");},200);
}
